...
from cfripper.config.whitelist import get_wildcard_principal_exemption_resource_list
...
class GenericWildcardPrincipal(Rule):
            ...
            for resource in resource_list:
                ...
                for policy in getattr(resource, "policies", []):
                    self.check_for_wildcards(
                        resource=policy,
                        resource_id=getattr(policy, "policy_name", resource.logical_id)
                    )
                # Resouce is a policy
                self.check_for_wildcards(
                    resource=resource,
    ...
    def resource_is_whitelisted(self, logical_id):
        return logical_id in get_wildcard_principal_exemption_resource_list(self._config.stack_name)
    ...
    def validate_account_id(self, account_id, logical_id):
        ...
        if (
            ...
            self.add_failure(
                type(self).__name__,
                self.REASON_NOT_ALLOWED_PRINCIPAL.format(logical_id, account_id),
            )
            logger.info(
